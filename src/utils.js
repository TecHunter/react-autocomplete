...
export const SearchAutocomplete = ({...props}) => {
	const [apiResult, fetchedFromApi] = useState([])
	return (
		<ReactAutocomplete
			{...props}
			items={apiResult}
			onSelect={action('select state')}
			getItemValue={({ abbr }) => abbr}
			onChange={(term) => fakeRequest(term, fetchedFromApi)}
			renderItem={({ abbr, name }, props) => <div {...props}>{abbr} - {name}</div>}
		/>
	)
}
